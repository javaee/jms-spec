...
Q | A
:--- | :---
Do the `context` variables in the three calls to `context.send()`  use the same injection point? | The first and third calls use the same injection point, since they use the same `context` variable in `Bean1`. The second call uses a different injection point since it uses a different variable in `Bean2`.
